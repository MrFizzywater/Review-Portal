...
import { Plus, Folder, LogOut, ExternalLink, Settings, Copy, Users, Edit2, Trash2, CopyPlus, Archive, ArchiveRestore, CheckCircle, Search, ChevronDown, ChevronUp } from 'lucide-react';
        ...
        {activeTab === 'projects' && (
          <>
            <div className="flex flex-col md:flex-row md:items-center justify-between mb-8 gap-4">
              <h1 className="text-2xl font-bold text-gray-900 dark:text-white shrink-0">Your Projects</h1>
              ...
              <div className="flex-1 max-w-md w-full relative">
                ...
                <input 
                  ...
                  className="w-full pl-10 pr-4 py-2 bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-xl text-sm focus:ring-2 focus:ring-black dark:focus:ring-white outline-none transition-all dark:text-white"
                />
              </div>
              ...
              if (activeProjects.length === 0 && !isCreatingProject) {
                return (
                  <div className="text-center py-12 bg-white dark:bg-gray-800 rounded-xl border border-dashed border-gray-300 dark:border-gray-700">
                    ...
                    <h3 className="text-gray-900 dark:text-white font-medium mb-1">No matches found</h3>
                    <p className="text-gray-500 dark:text-gray-400 text-sm">Try adjusting your search or create a new project.</p>
                  </div>
                );
              }
                            ...
                            {clientProjects.map(project => (
                              ...
                              >
                                ...
                                <Link to={`/project/${project.id}`} className="block mb-4">
                                  <h3 className="font-bold text-gray-900 dark:text-white text-lg mb-1 hover:text-blue-600 dark:hover:text-blue-400 transition-colors uppercase tracking-tight">{project.title}</h3>
                                </Link>
                                ...
                                <div className="space-y-3">
                                  <div className="flex items-center justify-between">
                                    ...
                                    <select
                                      ...
                                      className={`text-[10px] font-black uppercase px-2 py-1 rounded-md outline-none cursor-pointer tracking-wider ${
                                ...
                                {project.invoice && project.invoice.status !== 'draft' && (
                                  <div className="mt-4 pt-4 border-t border-gray-100 dark:border-gray-700 flex justify-between items-center text-xs">
                                    <span className="text-gray-500 dark:text-gray-400 font-bold uppercase tracking-wider">Invoice</span>
                                    {project.invoice.status === 'paid' || (project.invoice.total - (project.invoice.amountPaid || 0)) <= 0 ? (
                                      <span className="text-green-600 dark:text-green-400 font-black flex items-center gap-1 uppercase"><CheckCircle className="w-3 h-3"/> Paid</span>
                                    ) : (
                                      <span className="text-orange-600 dark:text-orange-400 font-black uppercase">
